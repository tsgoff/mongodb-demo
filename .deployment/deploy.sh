...
function deploy {
  ...
  DEPLOYID=$(aws deploy create-deployment --file-exists-behavior OVERWRITE --region eu-west-1 --application-name ${APP_NAME} --s3-location bucket=${S3BUCKET},key=${BUNDLEPATH}${BUNDLENAME},bundleType=tgz --deployment-group-name ${DG_NAME} --deployment-config-name CodeDeployDefault.OneAtATime --query 'deploymentId' | sed -e 's/^"//' -e 's/"$//')
  echo "triggered deployment"
  rm ${BUNDLENAME}
  echo "waiting for deployment ${DEPLOYID} to finish"
  aws deploy wait deployment-successful --region eu-west-1 --deployment-id ${DEPLOYID}
  aws deploy get-deployment --region eu-west-1 --deployment-id ${DEPLOYID}
